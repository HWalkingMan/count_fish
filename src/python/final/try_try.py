...
print('num:',len(contours_i2)) #计算连通域个数

img_dst=np.array(img_src)
img_dst_soimg=cv.cvtColor(img_gray,cv.COLOR_GRAY2BGR)


draw=cv.drawContours(img_dst,contours_i2,-1,(0,255,0),1) #描绘连通域
add_pic('dst',img_dst)

draw=cv.drawContours(img_dst_soimg,contours_i2,-1,(0,255,0),1) #描绘连通域
